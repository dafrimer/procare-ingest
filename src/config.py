...
class Config:
    ...
    api_url: str = field(default_factory=lambda: os.getenv("API_URL", "http://procare-api:8080"))
    ...
    def validate(self):
        ...
        if not self.ingest_token:
            raise ValueError("INGEST_TOKEN is required (must match procare-api's INGEST_TOKEN)")
        if not self.api_url:
